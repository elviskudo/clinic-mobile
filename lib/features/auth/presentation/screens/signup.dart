...
class SignUpScreen extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final mutation =
        useMutation<void, dynamic, RegistrationRequestBody, dynamic>(
      'auth/signup',
      (reqBody) => locator.get<AuthRepository>().register(reqBody),
      onData: (result, retry) {
        formKey.currentState!.reset();
        context.go('/app/verification');
      },
      onError: (error, recoveryData) {
        NativeToast().makeText(message: S.of(context).signUpError);
      },
                    ...
                    if (formKey.currentState!.validate()) {
                      await mutation.mutate(
                        RegistrationRequestBody(
                          ...
                          fullName: name.text,
                          phoneNumber: phoneNumber.value.parseNumber(),
                        ),
                      );
                    }
                  },
