...
class HomeController extends GetxController {
  ...
  // ==========================================================
  // FIX: TEKS STATUS DISESUAIKAN DENGAN INSTRUKSI GAMBAR
  // ==========================================================
  String getStatusText(int status) {
    switch (status) {
      ...
      case 6:
        return 'Unpaid';
      case 7:
        return 'Get Medicine'; // atau Rejected
      case 8:
        return 'Completed';
      default:
  ...
  // ==========================================================
  // FIX: ROUTING DISESUAIKAN DENGAN INSTRUKSI GAMBAR
  // ==========================================================
  // ==========================================================
  // FIX: FUNGSI NAVIGASI DIBUAT ASYNC AGAR BISA BACA MEMORI HP
  // ==========================================================
  Future<void> navigateToAppointmentDetail1(Appointment appointment) async {
    print(
        'Navigating for Appointment ID: ${appointment.id} with status: ${appointment.status}');

    // 1. Ambil nama langsung dari memori saat kartu diklik!
    final prefs = await SharedPreferences.getInstance();

    // 2. Kita cek dua key yang mungkin dipakai aplikasimu ('userName' atau 'name')
    String finalPatientName =
        prefs.getString('userName') ?? prefs.getString('name') ?? '';

    // 3. Fallback Mutlak: Jika masih kosong juga, kita paksa isi teks ini!
    if (finalPatientName.trim().isEmpty) {
      finalPatientName = 'Pasien Clinic AI';
    }

    switch (appointment.status) {
      case 1:
        Get.toNamed(Routes.SCHEDULE_APPOINTMENT, arguments: appointment.id);
        break;

      case 2:
        Get.toNamed(Routes.SYMPTOM_APPOINTMENT, arguments: appointment.id);
        break;

      case 3:
        Get.snackbar('Informasi', 'Mohon maaf, janji temu ditolak.',
      ...
      case 7:
        // FIX: MENGIRIM ARGUMEN LENGKAP KE HALAMAN PAYMENT SUCCESS
        Get.toNamed(Routes.PAYMENT_SUCCESS, arguments: {
          'qr_code': appointment.id.substring(0, 8).toUpperCase(),
          'patient_name':
              finalPatientName, // <- Dijamin tidak akan kosong lagi!
        });
        break;

      case 8:
        Get.toNamed(Routes.GRAB_MEDICINE, arguments: appointment.id);
        break;
